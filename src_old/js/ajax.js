...
function checkRidersLocations (rideId) { 
  ajax(rideId, peloBaseUrl + "last-known-location/"+rideId + "?since=" + new Date((new Date().getTime()-1000*60*60*24*15)).toISOString(), function (name, xhttp) { 
  switch (xhttp.status) {
    case 200:
      //debug(name,xhttp);
...
function updateUserLocation(userId,rideId,pos) {
  ...
  ajax(rideId, peloBaseUrl +"last-known-location/"+userId+"/"+rideId, function (rideId, xhttp) {
...
function rideRoute(rideId) { 
  ...
  ajax("ride_route" , peloBaseUrl + "ride-route/"+rideId,
...
function messages(userId) { 
  ajax("messages" , peloBaseUrl + "messages/"+userId,
...
function images() {
  ajax("userimages", peloBaseUrl + "userimage;id=1;id=2",//+getUserId(),
      function (name,xhttp) {
      switch (xhttp.status) {
      case 200:
      var data = storeJSON(name,xhttp.responseText);
...
function groups() {
  ajax("groups", peloBaseUrl + "groups/7",//+getUserId(),
      function (name,xhttp) {
      switch (xhttp.status) {
      case 200:
      var data = storeJSON(name,xhttp.responseText);
...
function todaysRides() {
  ajax("todays_rides", peloBaseUrl + "todays-rides/by-user/"+getUserId(),
      function (name,xhttp) {
      switch (xhttp.status) {
      case 200:
      var data = storeJSON(name,xhttp.responseText);
...
function login(username,password,callback){
  ajax("auth", peloBaseUrl + "login/"+username,function (name,xhttp) {
  switch (xhttp.status) {
    case 202:
...
function login2(username,password){
  ajax("auth", peloBaseUrl + "login2/"+username,function (name,xhttp) {
  switch (xhttp.status) {
    case 200:
      storeJSON(name,xhttp.responseText);
      doLogin();
